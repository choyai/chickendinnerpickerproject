...
def handleBag(ser, desired_type, bag_list, config, countsPerMillimeter, countsPerMillimeter_z, x_pixels_per_mil, y_pixels_per_mil, roi_width, roi_height):
    ...
    for i in range(len(bag_list)):
        ...
        if bag[1][1][0] > bag[1][1][1]:
            bag_angle = 90 + bag[1][2]
        else:
            bag_angle = 0 - bag[1][2]
        if bag[0] == desired_type and bag_y_grip < 160:
            print(bag[0] + " at " + str(int(bag_x_grip)) +
                  ", " + str(int(bag_y_grip)))
            goal = config[0]
        ...
        if bag[0] != desired_type and bag_y_grip < 160:
            ...
            setPosXY(int(360 * countsPerMillimeter),
                     0, ser)
            setPosZ(int(168 * countsPerMillimeter_z), ser)
            gripOpen(ser)
            setHome(ser)
...
bag_configs = {
    '1': [[337, 168, 168, 0], [200, 168, 168, 180], [337, 168, 166, 0], [200, 168, 166, 180], [337, 168, 144, 0], [188, 168, 144, 180]],
    '2': [[322, 368, 168, 0], [228, 262, 168, 90], [188, 168, 168, 180], [292, 130, 168, 270]],
    '3': [[188, 168, 188, 180], [188, 206, 166, 180], [188, 244, 154, 180], [322, 168, 188, 0], [322, 206, 166, 0], [322, 206, 166, 0], [322, 244, 154, 0]],
}
...
while(1):
    ...
    if start == 1 and config is not []:
        # cv2.waitKey(0)
        noone = input('press enter to continue')
        if bag_list == []:

...
def detect_type(c, lowH, lowS, lowV, upH, upS, upV, thresh, lowblueH, lowblueS, lowblueV, upper_blueH, upper_blueS, upper_blueV, bluethresh):
    # c = big_box[2]
    ...
    if len(cnts) is not 0:
        ...
        for c in cnts:
            if cv2.contourArea(c) < min_area or cv2.contourArea(c) > max_area:
                continue
            detect_type(c, lowH, lowS, lowV, upH, upS, upV, thresh, lowblueH,
